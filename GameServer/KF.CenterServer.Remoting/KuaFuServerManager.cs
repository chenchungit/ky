    ...
    public static class KuaFuServerManager
    {
        ...
        public static bool CheckConfig()
        {
            do
            {
                ...
                string fileName = GetResourcePath("Config", ResourcePathTypes.GameRes);
                if (Directory.Exists(GetResourcePath("Config", ResourcePathTypes.GameRes)) && Directory.Exists(GetResourcePath("1", ResourcePathTypes.Map))
                    && Directory.Exists(GetResourcePath("1", ResourcePathTypes.MapConfig)) && Directory.Exists(GetResourcePath("Config", ResourcePathTypes.Isolate)))
                {
                    break;
                }
            ...
            string filePath = GetResourcePath("Config/Settings.xml", ResourcePathTypes.GameRes);
            ...
            filePath = GetResourcePath("Config/SystemTasks.xml", ResourcePathTypes.Isolate);
            if (!File.Exists(filePath))
            {
                Console.WriteLine("错误:文件未找到.{0}", filePath);
                return false;
