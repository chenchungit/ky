        ...
        public bool WriteData(byte[] buffer, int offset, int count)
        {
            ...
            lock (mutex)
            {
                ...
                else
                {
                    ...
                    if (_Socket == null)
                    {
                        LogManager.WriteLog(LogTypes.Error, string.Format("TcpInPacket.WriteData 检查到socket为null, 可能是别的线程关闭了该socket后重置TcpInPacket, 当前消息(极有可能不准):{0}[{1}]", (TCPGameServerCmds)_PacketCmdID, _PacketCmdID));
                    }
