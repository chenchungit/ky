...
namespace Server.Tools
{
    ...
    public class DataHelper
    {
        ...
        /// <param name="cmdID"></param>
        /// <returns></returns>
        public static TCPOutPacket ProtocolToTCPOutPacket<T>(T instance, TCPOutPacketPool pool, int cmdID)
        {
            MemoryStream msResult = new MemoryStream();
            Serializer.Serialize<T>(msResult, instance);
            byte[] msSendData = msResult.ToArray();
            msResult.Dispose();
            msResult = null;
            return TCPOutPacket.MakeTCPOutPacket(pool, msSendData, 0, msSendData.Length, cmdID);
        }
