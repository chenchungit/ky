...
namespace GameServer.Logic
{
    ...
    public class Monster : IObject
    {
        ...
        public Point Realive()
        {
            ...
            Point toPoint;
            if ((int)MonsterTypes.DaDao == MonsterType || (int)MonsterTypes.DaDao == MonsterType) //如果是通天大盗或者夺宝奇兵怪物则，满地图随机复活
            {
                toPoint = Global.GetRandomPoint(ObjectTypes.OT_MONSTER, MonsterZoneNode.MapCode);
            }
            else
            {
        ...
        public string BuildAttackerLog()
        {
            ...
            sb.AppendFormat("怪物伤害日志: MonsterId={0}, MonsterName={1},", MonsterInfo.ExtensionID, Global.GetMonsterNameByID(MonsterInfo.ExtensionID));
            lock (_AttackerLogDict)
            {
        ...
        public virtual void Timer_Tick(object sender, EventArgs e)
        {
            ...
            {                
                if ((nowTicks - _LastActionTick) >= Global.MovingNeedTicksPerGrid)
                {
        ...
        /// 攻击间隔
        /// </summary>
        private long _MaxAttackTimeSlot = 2000;
