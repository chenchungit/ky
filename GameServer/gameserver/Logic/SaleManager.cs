...
namespace GameServer.Logic
{
    ...
    public class SaleGoodsMoneyCompare : IComparer<SaleGoodsData>
    {
        ...
        public int Compare(SaleGoodsData x, SaleGoodsData y)
        {
            ...
            return x.SalingGoodsData.SaleMoney1 - y.SalingGoodsData.SaleMoney1;
        }
    }
    ...
    public class SaleGoodsMoneyCompare2 : IComparer<SaleGoodsData>
    {
        ...
        public int Compare(SaleGoodsData x, SaleGoodsData y)
        {
            ...
            return y.SalingGoodsData.SaleMoney1 - x.SalingGoodsData.SaleMoney1;
        }
    }
    ...
    public class SaleGoodsNameAndColorCompare : IComparer<SaleGoodsData>
    {
        ...
        {
            int ret = 0;
            ret = x.SalingGoodsData.GoodsID - y.SalingGoodsData.GoodsID;
            if (ret == 0)
            {
    ...
    public class SaleGoodsSuitCompare : IComparer<SaleGoodsData>
    {
        ...
        public int Compare(SaleGoodsData x, SaleGoodsData y)
        {
            ...
            ret = Global.GetEquipGoodsSuitID(x.SalingGoodsData.GoodsID) - Global.GetEquipGoodsSuitID(y.SalingGoodsData.GoodsID);
            if (Desc)
            {
                ret = -ret;
            }
    ...
    public class SaleManager : IManager
    {
        ...
        {
            MaxSaleNum = (int)GameManager.systemParamsList.GetParamValueIntByName("ShangJiaNumber");
            double[] jiaoYiShuiParams = GameManager.systemParamsList.GetParamValueDoubleArrayByName("JiaoYiShui");
            if (null != jiaoYiShuiParams && jiaoYiShuiParams.Length == 2)
            {
                JiaoYiShuiJinBi = jiaoYiShuiParams[0];
                JiaoYiShuiZuanShi = jiaoYiShuiParams[1];
            }
            long maxSaleGoodsTime = GameManager.systemParamsList.GetParamValueIntByName("ShangJiaTime") * 1000;
            if (maxSaleGoodsTime > 0)
            {
                MaxSaleGoodsTime = maxSaleGoodsTime;
            }
        ...
        public static int[] GetTypeAndID(int goodsID)
        {
            ...
            lock (_GoodsID2TabIDDict)
            {
                if (!_GoodsID2TabIDDict.TryGetValue(goodsID, out typeAndID))
                {
                    ...
                    if (_Categoriy2TabIDDict.TryGetValue((occupation  * 100000000) + category, out typeAndID))
                    //if (_Categoriy2TabIDDict.TryGetValue((occupation << 32) + category, out typeAndID))
                    {
                        //添加到_GoodsID2TabIDDict字典缓存,下次就可以直接用了,不过这也导致需要lock了
                        _GoodsID2TabIDDict.Add(goodsID, typeAndID);
                        return typeAndID;
                    }
                }
            }
